...
const InfoIcon = React.memo<IconProps>(({ className }) => (
...
// Success Icon SVG
const SuccessIcon = React.memo<IconProps>(({ className }) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
// Warning Icon SVG
const WarningIcon = React.memo<IconProps>(({ className }) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
// Error Icon SVG
const ErrorIcon = React.memo<IconProps>(({ className }) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
// Loading Spinner SVG
const LoadingSpinner = React.memo<IconProps>(({ className }) => (
...
// A map to store styles and icon components for each notification type
...
const Notification: React.FC<NotificationProps> = ({ type, title, message, showIcon = true, duration, onClose }) => {
  ...
  // Auto-close effect for duration-based notifications
  React.useEffect(() => {
  ...
  }, [duration, type, onClose, isClosed]);
  ...
  return (
    ...
    >
        ...
        {duration && type !== 'loading' && ( // Render progress bar only if duration is provided and not loading
